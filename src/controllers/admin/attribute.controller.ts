...
export class AttributeController {
    ...
    async create(
        ...
        try {
            const doc = new Attribute();
            if (body.name !== undefined) {
                const exists = await this.repo.findOne({ where: { name: { $regex: new RegExp(`^${body.name.trim()}$`, "i") }, isDelete: 0 } });
                if (exists) return response(res, StatusCodes.BAD_REQUEST, "Attribute with this name already exists");
                doc.name = body.name;
            }
            if (body.values !== undefined) {
                doc.values = body.values.map((v: any) => ({
                    ...v,
                    isFilter: v.isFilter ?? false,
                    _id: (v._id && ObjectId.isValid(v._id)) ? v._id : new ObjectId().toString(),
                }));
            }
            ...
            return response(res, StatusCodes.CREATED, "Attribute created successfully", doc);
        } catch (error: any) {
    ...
    async edit(
        ...
        try {
            ...
            if (!doc) return response(res, StatusCodes.NOT_FOUND, "Attribute not found");
            ...
            await this.repo.save(doc);
            return response(res, StatusCodes.OK, "Attribute updated successfully", doc);
        } catch (error: any) {
            return handleErrorResponse(error, res);
        }
    }
    ...
    async delete(@Param("id") id: string, @Req() req: RequestWithFiles, @Res() res: Response) {
        try {
            ...
            return response(res, StatusCodes.OK, "Attribute deleted successfully");
        } catch (error: any) {
            return handleErrorResponse(error, res);
        }
    }
